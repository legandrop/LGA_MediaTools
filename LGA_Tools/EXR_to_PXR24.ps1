...
$sourcePath = $args[0]
if (-Not $sourcePath) {
    Write-Host "Error: Hay que arrastrar una carpeta con EXRs al archivo EXR_to_Pxr24.bat"
    Write-Host "Presione ESC para salir"
    while ($true) { if ($Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown").VirtualKeyCode -eq 27) { exit } }
}
...
if ($sourceName -like "*piz*") {
    $destName = $sourceName -replace "piz", "pxr24"
} else {
    $destName = "$sourceName-pxr24"
}
...
foreach ($file in $files) {
    ...
    $newFileName = if ($fileName -like "*piz*") { $fileName -replace "piz", "pxr24" } else { $fileName }
    ...
    $arguments = """$($file.FullName)"" --compression pxr24 -o ""$outputPath"""
