...
foreach ($file in $files) {
    ...
    foreach ($group in $groupedChannels.GetEnumerator()) {
        ...
        # Reemplazar "piz" en el nombre del archivo por la compresión correspondiente o agregar "-dwaa" o "-pxr24" según corresponda
        if ($fileName -like "*piz*") {
            $fileName = $fileName -replace 'piz', $compression
        } else {
