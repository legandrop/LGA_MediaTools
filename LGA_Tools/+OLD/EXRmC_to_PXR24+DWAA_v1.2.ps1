...
foreach ($file in $files) {
    ...
    foreach ($group in $groupedChannels.GetEnumerator()) {
        ...
        # Reemplazar: "FinalImagePPM_MRQ_05_SceneDepth" por "MRQ_SceneDepth"
        if ($baseName -eq "FinalImagePPM_MRQ_05_SceneDepth") {
            $baseName = "MRQ_SceneDepth"
        }

        # Crear el nombre base para la carpeta, reemplazando "piz" por la compresión correspondiente y agregando el canal al final
        if ($sourceName -like "*piz*") {
            $sourceNameModified = $sourceName -replace 'piz', $compression
        } else {
            $sourceNameModified = "$sourceName-$compression"
        }

        $newName = "$sourceNameModified-$baseName"
        ...
        # Renombrado de archivos según presencia de "zip" o "piz"
        if ($fileName -match "zip|piz") {
            # Reemplazar "zip" o "piz" con el codec correspondiente
            $outputFileName = $fileName -replace "zip|piz", $compression

            # Manejar posibles números de frame si existen
            $lastSeparatorIndex = $outputFileName.LastIndexOfAny(@('_', '.'))
            
            if ($lastSeparatorIndex -ge 0) {
                $possibleFrame = $outputFileName.Substring($lastSeparatorIndex + 1)
                if ($possibleFrame -match '^\d+$') {
                    $baseFileName = $outputFileName.Substring(0, $lastSeparatorIndex)
                    $frameNumber = $outputFileName.Substring($lastSeparatorIndex)
                    $outputFile = Join-Path $outputDirChannels "$baseFileName-$baseName$frameNumber.exr"
                } else {
                    $outputFile = Join-Path $outputDirChannels "$outputFileName-$baseName.exr"
                }
            } else {
                $outputFile = Join-Path $outputDirChannels "$outputFileName-$baseName.exr"
            }
        } else {
            # Agregar el codec antes del nombre del canal
            $outputFileName = "$sourceNameModified-$compression-$baseName"
            ...
            if ($lastSeparatorIndex -ge 0) {
                ...
                if ($possibleFrame -match '^\d+$') {
                    ...
                    $outputFile = Join-Path $outputDirChannels "$baseFileName-$compression-$baseName$frameNumber.exr"
                } else {
                    $outputFile = Join-Path $outputDirChannels "$outputFileName.exr"
                }
            } else {
                $outputFile = Join-Path $outputDirChannels "$outputFileName.exr"
            }
        ...
        $arguments = """$($file.FullName)"" --ch $channelList --compression $compression -o ""$outputFile"""
...
$uniqueFolders = $groupedChannels.Keys | ForEach-Object {
    ...
    # Determinar la compresión basada en el canal
    $compression = if ($baseName -eq "RGBA") { "dwaa" } else { "pxr24" }
