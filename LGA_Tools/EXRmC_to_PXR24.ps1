...
# Función para obtener los nombres de los canales, sin los subcanales .R, .G, .B, .A
function Get-ChannelNames {
    param ($inputExr)
    & $exrheaderPath $inputExr | 
        Select-String -Pattern "^\s+(\w+)" |  # Capturar el canal base
        Where-Object { $_.Matches.Groups[1].Value -notmatch '^[ARGB]$' } | 
        ForEach-Object { $_.Matches.Groups[1].Value.Split('.')[0] } | 
...
foreach ($file in $files) {
    ...
    foreach ($group in $groupedChannels.GetEnumerator()) {
        ...
        # Nuevo reemplazo: "FinalImagePPM_MRQ_05_SceneDepth" por "MRQ_SceneDepth"
        ...
        if ($channelList) {
            ...
            # Buscar el último "_" o "." en el nombre del archivo antes de la extensión
            ...
            if ($lastSeparatorIndex -ge 0) {
                ...
                # Verificar si lo que sigue es un número (el frame)
                if ($possibleFrame -match '^\d+$') {
                    # Es un número de frame, insertamos el canal antes del número de frame
                    $baseFileName = $fileName.Substring(0, $lastSeparatorIndex)  # Nombre base sin el número de frame
                    $frameNumber = $fileName.Substring($lastSeparatorIndex)  # Incluye el separador y el número de frame

                    # Crear el nombre del archivo con el canal insertado antes del número de frame
                    $outputFile = Join-Path $outputDirChannels "$baseFileName-$baseName$frameNumber.exr"
                } else {
                    # No es un número de frame, agregamos el canal al final como en el script original
                    $outputFile = Join-Path $outputDirChannels "$fileName-$baseName.exr"
                }
            } else {
                # No se encontró ningún "_" o ".", se usa el nombre del archivo completo
                $outputFile = Join-Path $outputDirChannels "$fileName-$baseName.exr"
            }
            ...
            Write-Host "  No se encontraron canales para: $baseName"
        }
    }
}
