...
$sourcePath = $args[0]
if (-Not $sourcePath) {
    Write-Host "Error: Hay que arrastrar una carpeta con EXRs al archivo EXR_to_ZIP.bat"
    Write-Host "Presione ESC para salir"
    while ($true) { if ($Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown").VirtualKeyCode -eq 27) { exit } }
}
...
if ($sourceName -like "*dwaa*") {
    $destName = $sourceName -replace "dwaa", "zip"
} else {
    $destName = "$sourceName-zip"
}
...
foreach ($file in $files) {
    ...
    $newFileName = if ($fileName -like "*dwaa*") { $fileName -replace "dwaa", "zip" } else { $fileName }
    ...
    $arguments = """$($file.FullName)"" --compression zip -o ""$outputPath"""
