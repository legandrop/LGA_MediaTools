...
#     Los archivos procesados se guardan en la misma carpeta con nuevos nombres según las reglas.
#
#   Requisitos:
...
function Pause-AndExit {
    if ($DeleteTempFiles) {
        ...
        $tempFiles = @($thumbPath, $placaTemplateOutputPath)
        foreach ($file in $tempFiles) {
            if (Test-Path $file) {
                Remove-Item $file
                Write-Host "Archivo temporal borrado: $file"
            }
        }
        Write-ColorOutput "Archivos temporales borrados exitosamente." "Green"
    } else {
        Write-Host "Los archivos temporales no se han borrado."
    }
...
# Obtener las dimensiones exactas del video de entrada
...
$ffmpegArgs = @(
    ...
    "-i", $inputMovFile,
    "-filter_complex",
    "[1:v]scale=1920:$($newHeight):flags=lanczos,pad=1920:1080:(1920-iw)/2:(1080-ih)/2[scaled];
    color=black@0.5:s=1920x1080,split=2[bar1][bar2];
    [bar1]crop=iw:ceil((ih-(iw/2.35))/2):0:0[topbar];
    [bar2]crop=iw:ceil((ih-(iw/2.35))/2):0:ih-ceil((ih-(iw/2.35))/2)[bottombar];
    [scaled][topbar]overlay=0:0[withTopBar];
    [withTopBar][bottombar]overlay=0:H-h[withBars];
    [0:v][withBars]concat=n=2:v=1:a=0[v];
...
$ffmpegMxfArgs = @(
    ...
    "-i", $inputMovFile,
    "-filter_complex",
    "[1:v]scale=1920:$newHeight,pad=1920:1080:(1920-iw)/2:(1080-ih)/2[scaled];
